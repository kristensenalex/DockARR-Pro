...
# Version: 4.0
# Description: Complete backup of the entire ARR stack including configs, volumes, and data
...
Write-Host "   ARR STACK FULL BACKUP SYSTEM V3" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
$StopContainers = Read-Host "Do you want to stop containers for a consistent backup? (recommended) [Y/n]"
if ($StopContainers -ne 'n') {
    ...
    Write-Log "Containers are still running - backup may be inconsistent" "WARNING"
...
Write-Log "`nBacking up data folder structure..."
...
# 10. Restart containers if they were stopped
...
if ($Encrypt) {
    ...
    if (!(Test-Path $7zipPath)) {
        ...
        # Convert SecureString to plain text (for 7-Zip command only)
        $BSTR = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($Password)
        $PlainPassword = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($BSTR)

        # Encrypt with 7-Zip
        $EncryptedFile = "$BackupPath\$BackupName.7z"
        & $7zipPath a -p"$PlainPassword" -mhe=on -mx=9 $EncryptedFile $BackupFile
...
if (Test-Path $BackupFile) {
    ...
    try {
        if ($BackupFile.EndsWith('.zip')) {
            ...
            & $7zipPath t $BackupFile | Out-Null
...
# 16. Generate improved restore script
$RestoreScript = @'
...
Write-Log "`nBackup process completed!" "SUCCESS"
...
if (Test-Path $BackupFile) {
    if ($BackupFile.EndsWith('.zip')) {
        ...
        Write-Host "  [Encrypted archive - content protected]" -ForegroundColor Yellow
    }
}